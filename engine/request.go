...
// continuous-batching inference loop. Ported from vllm_mlx/request.py.
package engine
...
// Request is a single inference request tracked by the scheduler. Adapted from
// vLLM's Request, simplified for the MLX backend.
type Request struct {
	RequestID      string
	Prompt         string
