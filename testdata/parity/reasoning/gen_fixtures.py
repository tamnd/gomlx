...
"""
Generate golden parity fixtures for reasoning parsers from the Rapid-MLX
reference implementation. Run from the Rapid-MLX repo root (so `vllm_mlx`
imports), writing fixtures into the gomlx testdata dir:

    cd /Users/apple/github/raullenchai/Rapid-MLX
    python3 /Users/apple/github/tamnd/gomlx/testdata/parity/reasoning/gen_fixtures.py

Each fixture exercises both extract_reasoning (full output) and the streaming
path (delta-by-delta, then finalize), capturing every emitted DeltaMessage so
the Go port can be compared chunk-for-chunk.
"""

import json
import os

import vllm_mlx.reasoning as R
